...
const reducer = (state = initialState, { type, payload }) => {
  ...
  switch (type) {
    ...
    case "DELETE_COLLEGE":
      var filteredList2 = state.colleges.filter(
        (college) => college.collegeRegId !== payload.collegeRegId
      );
    ...
    case "DELETE_COURSE":
      console.log(payload);
      var filteredList1 = state.courses.filter(
        (course) => course.courseId !== payload.course.courseId
      );
    ...
    case "DELETE_BRANCH":
      filteredList = state.branches.filter(
        (branch) => branch.branchId !== payload.branch.branchId
      );
    ...
    case "DELETE_PROGRAM":
      filteredList = state.programs.filter(
        (program) => program.programId !== payload.program.programId
      );
    ...
    case "FIND_PROGRAMSCHEDULED":
      return {
        ...state,
        programSchedules: payload,
        updateProgramScheduled: payload.programScheduled,
      };
    ...
    case "DELETE_PROGRAMSCHEDULED":
      filteredList = state.programSchedules.filter(
        (programScheduled) =>
          programScheduled.scheduleId !== payload.programScheduled.scheduleId
      );
    ...
    case "DELETE_UNIVERSITY":
      var filteredList3 = state.universitys.filter(
        (university) => university.universityId !== payload.universityId
      );
